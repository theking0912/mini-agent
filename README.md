...
| 版本 | 新增能力 | 代码量估 | 状态 |
|------|---------|---------|------|
| v1 | 调 API，能调用 2-3 个工具 | ~300 行 | ✅ 已实现 |
| v2 | 上下文记忆（SQLite） | +100 行 | ❌ 待实现 |
| v3 | 多轮对话 + 工具链编排 | +150 行 | ✅ 已实现 |
| v4 | MCP 协议支持（动态注册工具） | +300 行 | ❌ 待实现 |
| v5 | **多模型 Key 适配 + 运行时切换** | +200 行 | ✅ 已实现 |
| v6 | **Web UI + 多用户 + 头像管理** | +800 行 | ✅ 已实现 |
| v7 | **RAG 知识库检索** | +400 行 | 🔜 计划中 |
| v8 | **Task Planning 任务规划** | +350 行 | 🔜 计划中 |
| v9 | **记忆持久化 + 跨会话上下文** | +250 行 | 🔜 计划中 |
| v10 | **多 Agent 协作 + 任务委派** | +500 行 | 🔜 计划中 |
